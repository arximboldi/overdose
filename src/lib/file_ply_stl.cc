...
int _file_ply::create(const char *File_name)
{
...
int _file_ply::is_number(const char *Aux_token)
{
  char *p1;

  for (size_t i=0;i<strlen(Aux_token);i++)
    {
        if ((p1=index((char*)"0123456789.+-eE",Aux_token[i]))==NULL) return(-1);
    }
  return(0);
}
...
int _file_ply::search_token(const char *Aux_token)
{
  int i=0;
